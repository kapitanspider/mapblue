...
<script>
var to_export=[["System Map Blue","Powiaty w okręgu nr <?php echo $_GET["okr"]?>","Od <?php echo $_GET["begin"]?>","Do <?php echo $_GET["end"]?>"],["Powiat",'ilość']]
  function download(elem){
    exportToCsv("Powiaty w okręgu nr <?php echo $_GET["okr"]?>", to_export);
  }
...
<?php
$sql = "SELECT count(id), powiat FROM `aktywnosci` Where data between '".$_GET["begin"]."' and '".$_GET["end"]."' and okreg = '".$_GET["okr"]."' GROUP by powiat order by count(id) desc";
$result = $conn->query($sql);
while($row = $result->fetch_assoc())
{
echo '
<script>
to_export.push(["'.$row['powiat'].'","'.$row['count(id)'].'"]);
</script>
<tr>
<td>'.$row['powiat'].'</td>
<td>'.$row['count(id)'].'</td>
...
$sql = "select powiat from powiaty where powiat not in (SELECT powiat FROM `aktywnosci` Where data between '".$_GET["begin"]."' and '".$_GET["end"]."' ) and okreg = '".$_GET["okr"]."' order by powiat asc";
$result = $conn->query($sql);
while($row = $result->fetch_assoc())
{
echo '
<script>
to_export.push(["'.$row['powiat'].'","0"]);
</script>
<tr>
<td>'.$row['powiat'].'</td>
<td>0</td>
