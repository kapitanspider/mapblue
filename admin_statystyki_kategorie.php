...
<script>
var to_export=[["System Map Blue","Rodzaje aktywnosci w skali krajowej","Od <?php echo $_GET["begin"]?>","Do <?php echo $_GET["end"]?>"],["Rodzaj",'ilość']];
  function download(elem){
    exportToCsv("Rodzaje aktywnosci skala krajowa", to_export);
  }
...
?>
<script type="text/javascript">
  var rodzaj="<?php echo $row["rodzaj"]; ?>";
  var ilosc="<?php echo $row["Count(ID)"]; ?>";
   xVal.push(rodzaj);
   yVal.push(ilosc);
   to_export.push([rodzaj,ilosc]);
</script>
<?php
}
?>
