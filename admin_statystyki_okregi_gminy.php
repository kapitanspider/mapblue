...
<script>
var to_export=[["System Map Blue","Gminy w okręgu nr <?php echo $_GET["okr"]?>","Od <?php echo $_GET["begin"]?>","Do <?php echo $_GET["end"]?>"],["Gmina",'ilość']]
  function download(elem){
    exportToCsv("Gminy w okręgu nr <?php echo $_GET["okr"]?>", to_export);
  }
...
<?php
$sql = "SELECT count(id), gmina FROM `aktywnosci` Where data between '".$_GET["begin"]."' and '".$_GET["end"]."'  and okreg = '".$_GET["okr"]."' GROUP by gmina order by count(id) desc";
...
$sql = "select gmina from gminy where gmina not in (SELECT gmina FROM `aktywnosci` Where data between '".$_GET["begin"]."' and '".$_GET["end"]."')  and okreg = '".$_GET["okr"]."' order by gmina asc";
$result = $conn->query($sql);
while($row = $result->fetch_assoc())
{
echo '
