...
<script>
var to_export=[["System Map Blue","Gminy w skali krajowej","Od <?php echo $_GET["begin"]?>","Do <?php echo $_GET["end"]?>"],["Gmina",'ilość']]
  function download(elem){
    exportToCsv("Gminy skala krajowa", to_export);
  }
...
<?php
$sql = "SELECT count(id), gmina FROM `aktywnosci` Where data between '".$_GET["begin"]."' and '".$_GET["end"]."' GROUP by gmina order by count(id) desc";
...
$sql = "select gmina from gminy where gmina not in (SELECT gmina FROM `aktywnosci` Where data between '".$_GET["begin"]."' and '".$_GET["end"]."') order by gmina asc";
$result = $conn->query($sql);
while($row = $result->fetch_assoc())
{
echo '
